...
bool Polynomial::MonomCompare::operator()(const Monom& m1, const Monom& m2) const noexcept {
	...
	auto iter_m1 = m1.variables_.begin();
	auto iter_m2 = m2.variables_.begin();

	while (iter_m1 != m1.variables_.end() && iter_m2 != m2.variables_.end()) {
	...
	return m1.coefficient_ < m2.coefficient_;
}
...
Polynomial::Monom& Polynomial::Monom::operator*=(const Monom& other) {
	...
	if (variables_.is_empty() && other.variables_.is_empty()) return *this; 
...
void Polynomial::normalize() {
	...
	while (it != polynomial_.cend()) {
		if (curr.is_similar(*it)) {
			curr.coefficient_ += it->coefficient_;
		}
		else {
			if (!curr.is_zero()) {
...
Polynomial& Polynomial::operator+=(const Polynomial& rhs) {
	...
	while (it1 != polynomial_.cend() && it2 != rhs.polynomial_.cend()) {
		if (MonomCompare{}(*it1, *it2)) {
			...
			Monom curr = *it1;
			curr.coefficient_ += it2->coefficient_;
			if (!curr.is_zero()) {
				result.insert(curr);
			}
			++it1;
...
Polynomial& Polynomial::operator-=(const Polynomial& rhs) {
	...
	for (const auto& monom : rhs.polynomial_) {
		Monom tmp = monom;
		tmp.coefficient_ = -tmp.coefficient_;
...
double Polynomial::calculate(const SortedList<VariableValue, VariableValueCompare>& values) const {
	...
	for (const auto& monom : polynomial_) {
		double term_value = monom.coefficient_;
		bool term_valid = true;

		for (const auto& var : monom.variables_) {
